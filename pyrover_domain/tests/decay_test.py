...
config_dir = Path(os.path.expanduser("/home/magraz/rovers/pyrover_domain/experiments/yamls/multi_agent_decay_mlp.yaml"))
...
for s in range(config["ccea"]["num_steps"]):
    ...
    for i, poi in enumerate(env.pois()):
        ...
        print(
            f"{s} POI: {i},  POS: {poi.position()},  VIS: {poi.visible}, SATISFIED: {poi.constraint_satisfied(pack)} VAL: {poi.value()}\n"
        )
